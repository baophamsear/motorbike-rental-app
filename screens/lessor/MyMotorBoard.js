...
const complaints = [
  ...
  { name: 'Cabin K.', complaint: 'Elivator shaking ...', severity: 'blue' },
  { name: 'Tylor S.', complaint: 'Parking area is ...', severity: 'orange' },
  { name: 'Micheal J.', complaint: 'No water since mon...', severity: 'red' },
];
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
        ...
        <View style={styles.infoRow}>
            ...
            onPress={() => nav.navigate('MotorManagement')}>  {/* green */}
            <Text style={styles.cardTitle}>My Bikes</Text>
            <View style={styles.circle}><Text style={styles.circleText}>8</Text></View>
            </TouchableOpacity >
            <TouchableOpacity style={[styles.card, { backgroundColor: '#f87171' }]}
            onPress={() => nav.navigate('ContractManagement')}>  {/* red */}
        ...
        {complaints.map((item, index) => (
            ...
            <View style={[styles.severityDot, { backgroundColor: item.severity }]} />
            </View>
        ))}

        <View style={styles.pagination}>
            {[1, 2, 3, 4, 5, 6].map(num => (
            <TouchableOpacity key={num} style={[styles.pageButton, num === 1 && styles.pageActive]}>
                <Text>{num}</Text>
            </TouchableOpacity>
            ))}
        </View>
