...
const NotificationsScreen = () => {
    ...
    const activeRental = renterId ? topics.renter.activeRental(renterId) : null;
    const completedRental = renterId ? topics.renter.completedRental(renterId) : null;
    ...
    const { messages: messagesActiveRental } = useWebSocket(activeRental);
    const { messages: messagesCompletedRental } = useWebSocket(completedRental);
    ...
    useEffect(() => {
        if (messagesActiveRental && messagesActiveRental.length > 0 ||
            (messagesCompletedRental && messagesCompletedRental.length > 0)
        ) {
            fetchNotifications();
        }   
    }, [messagesActiveRental, messagesCompletedRental]);
